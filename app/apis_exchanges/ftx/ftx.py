...
class FtxClient:
    ...
    def get_open_trigger_orders(self, market: str = None, type: str = None) -> List[dict]:
        return self._get(f'orders', {'market': market, 'type': type})
    ...
    def change_account_leverage(self, leverage: str) -> dict:
    ...
    def get_status_order(self, order_id: str) -> dict: 
        return self._get(f'orders/{order_id}')
    ...
    def get_leveraged_tokens(self) -> List[dict]:
        return self._get('lt/tokens')

    def get_token_info(self, token_name: str) -> dict:
        return self._get(f'lt/{token_name}')

    def get_orders_history(self, market: str, orderType: str) -> dict:
        return self._get(f'orders/history?market={market}&orderType={orderType}')
    ...
    def get_all_trades(self, market: str, start_time: float = None, end_time: float = None) -> List:
        ...
        while True:
            ...
            if len(response) == 0:
                break
            end_time = min(parser(t['time']) for t in response).timestamp()
